...
configtxgen -profile VTANOrdererGenesis -outputBlock ./channel-artifacts/genesis.block
configtxgen -profile VTANChannel -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID mainchannel
...
OPTS="-i"
cd $CURRENT_DIR
cd ./crypto-config/peerOrganizations/citizen.vtan.com/ca/
PRIV_KEY=$(ls *_sk)
cd $CURRENT_DIR
cd ./base
...
cd $CURRENT_DIR
cd ./crypto-config/peerOrganizations/rto.vtan.com/ca/
PRIV_KEY=$(ls *_sk)
cd $CURRENT_DIR
cd ./base
...
cd $CURRENT_DIR
cd ./crypto-config/peerOrganizations/pollution.vtan.com/ca/
PRIV_KEY=$(ls *_sk)
cd $CURRENT_DIR
cd ./base
...
cd $CURRENT_DIR
cd ./crypto-config/peerOrganizations/insurance.vtan.com/ca/
PRIV_KEY=$(ls *_sk)
cd $CURRENT_DIR
cd ./base
...
cd $CURRENT_DIR
cd ./crypto-config/peerOrganizations/police.vtan.com/ca/
PRIV_KEY=$(ls *_sk)
cd $CURRENT_DIR
cd ./base
